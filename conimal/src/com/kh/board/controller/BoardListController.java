...
public class BoardListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Adopt> adoptList = new BoardService().selectAdoptList(pi);
		ArrayList<Animal> animalList = new BoardService().selectAnimalList(pi);
		ArrayList<C_Post> c_postList = new BoardService().selectC_PostList(pi);
		ArrayList<Donation> doList = new BoardService().selectDonationList(pi);
		ArrayList<Volunteer> voList = new BoardService().selectVolunteerList(pi);
		
		request.setAttribute("pi", pi);
		request.setAttribute("adoptList", adoptList);
		request.setAttribute("animalList", animalList);
		request.setAttribute("c_postList", c_postList);
		request.setAttribute("doList", doList);
		request.setAttribute("voList", voList);
		
		if(adoptList != null && animalList!=null && c_postList!=null && doList!=null && voList!=null) { // 성공
